...
export function MagneticSocialBotWorkspace() {
  ...
  async function saveIntegration(integration: SocialBotIntegration, updates: Partial<SocialBotIntegration> & { accessToken?: string }) {
    ...
    setToast(`${integration.channel} saved.`);
    await loadWorkspace();
  }
  ...
  return (
    <div className="space-y-6">
        ...
        {selectedStep === 2 ? (
          <div className="mt-6 space-y-4">
            <div className="rounded-[24px] border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm text-cyan-900 dark:border-cyan-400/20 dark:bg-cyan-400/10 dark:text-cyan-100">
              No Meta developer account? Use our Meta app via embedded signup. Your customer only connects Facebook, picks a page or number, and authorizes access.
            </div>
            <div className="grid gap-4 lg:grid-cols-3">
              {integrations.map((integration) => (
                <IntegrationCard key={integration._id} integration={integration} onSave={saveIntegration} />
              ))}
            </div>
          </div>
        ) : null}
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <InfoCard title="Embedded signup" text="If your customer has no Meta developer account, they can still connect through your provider-owned Meta app." />
...
function IntegrationCard({ integration, onSave }: { integration: SocialBotIntegration; onSave: (integration: SocialBotIntegration, updates: Partial<SocialBotIntegration> & { accessToken?: string }) => Promise<void> }) {
  ...
  return (
    <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
      ...
      <div className="mt-4 grid gap-3">
        ...
        <input value={accessToken} onChange={(event) => setAccessToken(event.target.value)} placeholder="Access Token" className="h-11 w-full rounded-[18px] border border-slate-200 bg-white px-4 text-sm text-slate-950 outline-none dark:border-white/10 dark:bg-slate-950/60 dark:text-white" />
