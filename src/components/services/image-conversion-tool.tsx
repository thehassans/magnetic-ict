...
export function ImageConversionTool({ compact = false }: { compact?: boolean }) {
  ...
  const defaultMessage = compact ? "Drop image." : "Upload an image to convert formats and resize dimensions instantly.";
  ...
  function handleFile(file: File | null) {
    ...
    setError(null);
    setMessage(compact ? "Ready." : "Image ready. Choose the output format, resize options, and start conversion.");
  }

  async function processImage() {
    if (!selectedFile) {
      setError("Upload an image before starting conversion.");
      setMessage(compact ? "Add image." : "Choose a file to continue.");
      return;
    }

    setIsProcessing(true);
    setError(null);
    setMessage(compact ? "Processing..." : "Processing image, applying resize rules, and preparing your download.");
    ...
    try {
      ...
      setMessage(compact ? "Done." : "Conversion complete. Preview the output and download the processed image.");
      setResultInfo({
        ...
        <div className={cn("rounded-[36px] border border-violet-100 bg-white/90 shadow-glow backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/70", compact ? "p-4 sm:p-5" : "p-6 sm:p-8")}>
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-xl">
                {compact ? (
                  <div className="space-y-2">
                    <div className="text-sm font-medium text-slate-950 dark:text-white">{selectedFile?.name ?? "Upload"}</div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  ...
                  {isProcessing ? "Processing" : compact ? "Convert" : "Convert image"}
                </button>
                <button
                  type="button"
                  onClick={resetAll}
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <div className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{compact ? "Format" : "Output format"}</div>
            ...
            <div className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{compact ? "Fit" : "Resize mode"}</div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <label className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{compact ? "W" : "Width"}</div>
              <input
                inputMode="numeric"
                value={width}
            ...
            <label className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{compact ? "H" : "Height"}</div>
              <input
                inputMode="numeric"
                value={height}
            ...
            <label className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{compact ? "Q" : "Quality"}</div>
              <input
                type="range"
                min={40}
                max={100}
        ...
        <div className={cn("rounded-[36px] border border-violet-100 bg-white/90 shadow-glow backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/70", compact ? "p-4 sm:p-5" : "p-6 sm:p-8")}>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className={cn("font-semibold text-slate-950 dark:text-white", compact ? "text-lg" : "mt-3 text-2xl")}>{compact ? "Preview" : "Original and output"}</h2>
            </div>
            {resultBlob ? (
              <button
                type="button"
          ...
          <div className="mt-6 space-y-4">
            <PreviewCard
              title={compact ? "Before" : "Original image"}
              subtitle={sourceInfo ? `${sourceInfo.width} × ${sourceInfo.height}px` : compact ? "-" : "Upload a file to preview it here."}
              imageUrl={sourcePreviewUrl}
              fallbackLabel={compact ? "B" : "Original"}
            />
            <AnimatePresence initial={false}>
              <motion.div layout>
                <PreviewCard
                  title={compact ? "After" : "Processed output"}
                  subtitle={resultInfo ? `${resultInfo.width} × ${resultInfo.height}px${resultFileName ? ` • ${resultFileName}` : ""}` : compact ? "-" : "Run conversion to generate a new file preview."}
                  imageUrl={resultPreviewUrl}
                  fallbackLabel={compact ? "A" : "Output"}
                />
              </motion.div>
            </AnimatePresence>
          </div>
